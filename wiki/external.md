...
These retractions cascade from dependencies on claims about multi-agent safety spanning all layers (`[multi-agent-safety-spans-all-layers](agent.md#multi-agent-safety-spans-all-layers)`) and architectural enforcement properties that no longer hold. The foundational integration and lifecycle mechanisms remain intact — it is the comprehensive safety *guarantees* layered atop them that have been withdrawn. The system still integrates external beliefs through validated pipelines; it simply no longer claims that those pipelines are provably airtight across every possible dimension.

The retraction of `ask-degrades-across-all-external-dependencies` similarly reflects a gap: while the `ask` module handles LLM binary failures and FTS5 index issues gracefully, the claim of complete graceful degradation across *all* external dependencies lost support when the sources-database fault tolerance claim (`[ask-sources-db-failure-silently-degrades](other.md#ask-sources-db-failure-silently-degrades)`) was retracted.
